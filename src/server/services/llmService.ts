...
Format response STRICTLY as valid JSON matching this schema (do NOT include markdown formatting or backticks outside the JSON):
{
  ...
  if (geminiApiKey) {
    try {
      ...
      return {
        urgencyLevel: (parsed.urgencyLevel?.toUpperCase() as 'LOW' | 'MEDIUM' | 'HIGH') || 'MEDIUM',
      ...
      console.warn('Gemini API call failed, using graceful AI fallback:', (err as Error).message);
    }
    ...
    try {
      ...
      console.warn('OpenAI API call failed, using graceful AI fallback:', (err as Error).message);
    }
  }
 ...
 */
export async function generatePostVisitSummary(notes: string, prescription: string): Promise<PostVisitAnalysis> {
  const geminiApiKey = process.env.GEMINI_API_KEY;
  const openaiApiKey = process.env.OPENAI_API_KEY;
...
Format response STRICTLY as valid JSON matching this schema:
{
  ...
  if (geminiApiKey) {
    try {
      ...
      console.warn('Gemini API post-visit call failed, using fallback:', (err as Error).message);
    }
  } else if (openaiApiKey) {
    try {
      const OpenAI = (await import('openai')).default;
      ...
      console.warn('OpenAI API post-visit call failed, using fallback:', (err as Error).message);
    }
  }
...
function generateFallbackPreVisit(symptoms: string): PreVisitAnalysis {
...
function generateFallbackPostVisit(notes: string, prescription: string): PostVisitAnalysis {
  ...
  const medsList: MedicationScheduleItem[] = [];
