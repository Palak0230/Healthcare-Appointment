 ...
 */
export async function createGoogleCalendarEvent(payload: CalendarEventPayload): Promise<string> {
  if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_REFRESH_TOKEN) {
    try {
      ...
      console.warn('[GOOGLE CALENDAR API WARNING] Could not insert event to Google API:', (err as Error).message);
    }
  }
...
export async function deleteGoogleCalendarEvent(eventId: string): Promise<boolean> {
  ...
  if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_REFRESH_TOKEN && !eventId.startsWith('mock_')) {
    try {
      ...
      console.warn('[GOOGLE CALENDAR API WARNING] Failed deleting event:', (err as Error).message);
    }
  }
...
export async function updateGoogleCalendarEvent(
  ...
  if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_REFRESH_TOKEN && !eventId.startsWith('mock_')) {
    try {
      ...
      console.warn('[GOOGLE CALENDAR API WARNING] Failed updating event:', (err as Error).message);
    }
  }
